...
var dataPath = '/tmp/geoip-data'
var tmpPath = '/tmp/geoip-tmp'

function consoleLog() {
	if (process.env.GEOIP_STD_OUT_ENABLED) {
...
function getDatabases(databaseUrl=null) {
	var databases = [
		{
			type: 'country',
			url: (databaseUrl || 'https://geolite.maxmind.com/download/geoip/database/') + 'GeoIPCountryCSV.zip',
			src: 'GeoIPCountryWhois.csv',
			dest: 'geoip-country.dat'
		},
		{
			type: 'country',
			url: (databaseUrl || 'https://geolite.maxmind.com/download/geoip/database/') + 'GeoIPv6.csv.gz',
			src: 'GeoIPv6.csv',
			dest: 'geoip-country6.dat'
		},
		{
			type: 'city-extended',
			url: (databaseUrl || 'https://geolite.maxmind.com/download/geoip/database/GeoLiteCity_CSV/') + 'GeoLiteCity-latest.zip',
			src: [
		...
		{
			type: 'city',
			url: (databaseUrl || 'https://geolite.maxmind.com/download/geoip/database/GeoLiteCityv6-beta/') + 'GeoLiteCityv6.csv.gz',
...
function fetch(database, cb) {
	...
	consoleLog('Fetching ', downloadUrl);
	...
	function onResponse(response) {
		...
		if (status !== 200) {
			consoleLog('ERROR'.red + ': HTTP Request Failed [%d %s]', status, https.STATUS_CODES[status]);
			client.abort();
			process.exit();
		}
...
function processCountryData(src, dest, cb) {
	...
	stdOut('Processing Data (may take a moment) ...');
	var tstart = Date.now();
...
function updateDb(databaseUrl, cb) {

	rimraf(tmpPath);
	mkdir(tmpPath);

	async.eachSeries(getDatabases(databaseUrl), function(database, nextDatabase) {
		async.seq(fetch, extract, processData)(database, nextDatabase);

	}, function(err) {
		if (err) {
			consoleLog('Failed to Update Databases from MaxMind.'.red);
		} else {
			consoleLog('Successfully Updated Databases from MaxMind.'.green);
			if (process.argv[2]=='debug' || process.argv[3]=='debug') consoleLog('Notice: temporary files are not deleted for debug purposes.'.bold.yellow);
			else rimraf(tmpPath);
		}
		cb(err)
	});
